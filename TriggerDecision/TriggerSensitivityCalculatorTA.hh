...
public:
  TriggerSensitivityCalculator(const int         nSNEvent               ,
    ...
    std::cout << "Starting up TSC" << std::endl;
    SignalPDF_     = (TH1D*)InputSignalFile_    ->Get(Form("PDF_Signal"));
    BackgroundPDF_ = (TH1D*)InputBackgroundFile_->Get(Form("PDF_Background"));
    std::cout << "Got PDFs" << std::endl;

    int test = SignalPDF_->GetMaximum();
    ...
    std::cout << "Check integrals" << std::endl;
