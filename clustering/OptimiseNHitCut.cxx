...
int main(int argc, char** argv){
  ...
  TH1D* h_ncluster_sign_opti = new TH1D("h_ncluster_sign_opti", ";n clusters;Events", 10, 0, 10);
  TH1D* h_ncluster_back_opti = new TH1D("h_ncluster_back_opti", ";n clusters;Events", 10, 0, 10);
  ...
  TH1D* h_nhit_sign_opti = new TH1D("h_nhit_sign_opti", ";n Hits;Clusters", 300, 0, 300);
  TH1D* h_nhit_back_opti = new TH1D("h_nhit_back_opti", ";n Hits;Clusters", 300, 0, 300);
  ...
  //TF1 *f1 = new TF1("double_exp","[0]*exp([1]*x)+[2]*exp([3]*x)",0,400);
  TF1 *f1 = new TF1("double_exp","[0]*exp([1]*x)",0,300);
  f1->SetParameter(0, 1e8);
  f1->SetParameter(1, -10);
  // f1->SetParameter(2, 2e5);
  // f1->SetParameter(3, -20);
  double threshold = GetStatThreshold(h_nhit_sign_opti,h_nhit_back_opti);
  std::cout << "threshold " << threshold << std::endl;
  //h_nhit_back_opti->Fit("double_exp", "", "", 10, GetStatThreshold(h_nhit_sign_opti,h_nhit_back_opti));
  h_nhit_back_opti->Fit("double_exp");

  TH1D* h_npe_sign_opti = new TH1D("h_npe_sign_opti", ";n PEs;Clusters", 400, 0, 400);
  TH1D* h_npe_back_opti = new TH1D("h_npe_back_opti", ";n PEs;Clusters", 400, 0, 400);
  ...
  std::vector <double> max = {h_npe_sign_opti->GetMaximum(),
  ...
  TH1D* h_nhit_LMCEff = new TH1D("h_nhit_LMCEff", ";n Hits cut;LMC efficiency", 300, 0, 300);
  //for (int nHitCut=30; nHitCut<GetStatThreshold(h_nhit_sign_opti,h_nhit_back_opti); nHitCut++) {
  for (int nHitCut=50; nHitCut<51; nHitCut++) {
    ...
    for (int iEntry=0; iEntry<t_ClusteredOptHit->GetEntries()/Divisor; iEntry++) {
      ...
      if (Event==0 && !resetfirst && iEntry!=0) {
        resetfirst=true;
    ...
    for (auto const& it: totalBackCluster) h_ncluster_back_opti->Fill(it.second);
    for (auto const& it: totalSignCluster) h_ncluster_sign_opti->Fill(it.second);
    ...
    std::cout << "Background rate " << (double)nBackgroundCluster / (double)nTotalEvent / 4.492e-3 / 0.12 << std::endl;
    std::cout << "Background rate2 " <<  h_nhit_back_opti->Integral(nHitCut+1, 301) / (double)nTotalEvent / 4.492e-3 / 0.12 << std::endl;
    std::cout << "Background rate3 " <<  f1->Integral(nHitCut+1, 301) / (double)nTotalEvent / 4.492e-3 / 0.12 << std::endl;
  ...
  h_nhit_sign_opti->GetXaxis()->SetTitleSize(h_nhit_sign_opti->GetXaxis()->GetTitleSize()/.7);
  h_nhit_sign_opti->GetXaxis()->SetLabelSize(h_nhit_sign_opti->GetXaxis()->GetLabelSize()/.7);
  h_nhit_sign_opti->GetYaxis()->SetTitleSize(h_nhit_sign_opti->GetYaxis()->GetTitleSize()/.7);
  h_nhit_sign_opti->GetYaxis()->SetLabelSize(h_nhit_sign_opti->GetYaxis()->GetLabelSize()/.7);
  ...
  pad2->cd();
  pad2->SetTopMargin(0);
  pad2->SetBottomMargin(pad2->GetBottomMargin()/0.3);
  h_nhit_LMCEff->SetStats(0);
  h_nhit_LMCEff->GetXaxis()->SetTitleSize(h_nhit_LMCEff->GetXaxis()->GetTitleSize()/.3);
  h_nhit_LMCEff->GetXaxis()->SetLabelSize(h_nhit_LMCEff->GetXaxis()->GetLabelSize()/.3);
  h_nhit_LMCEff->GetYaxis()->SetTitleSize(h_nhit_LMCEff->GetYaxis()->GetTitleSize()/.3);
  h_nhit_LMCEff->GetYaxis()->SetTitleOffset(h_nhit_LMCEff->GetYaxis()->GetTitleOffset()*.3);
  h_nhit_LMCEff->GetYaxis()->SetNoExponent();
  h_nhit_LMCEff->GetYaxis()->SetLabelSize(h_nhit_LMCEff->GetYaxis()->GetLabelSize()/.3);
  h_nhit_LMCEff->Draw();
